...
- `codex`
  - idle but alive: 최근 `<= 90s` 내 strong heartbeat 없음, process alive
  - suspected stall: `> 90s` strong heartbeat 없음
  - hard stall: `> 180s` strong heartbeat 없음 and recovery 조건 충족
...
- `step1`/`step2`처럼 side-effect-free인 read-only 단계에서만 same session handle로 동일 step message를 한 번 더 `resume`한다.
- `step3`처럼 code apply/commit/push가 가능한 단계는 재전송 전에 worktree diff, `journal.commit_sha`, round `step3` state를 먼저 확인해 이미 mutation이 발생했는지 reconcile한다.
- mutation 단계에 들어갈 때 orchestrator는 stable `step_id`를 발급하고, `{round, step, step_id}`에서 파생한 `dedupe_token`을 만든 뒤 현재 step trace와 recovery payload 양쪽에 기록한다.
...
- read-only 단계만 현재 step message를 재전송한다.
- mutation 가능 단계는 새 세션에도 같은 step message를 바로 재전송하지 않고, recovery payload에 `step_id`/dedupe token과 reconcile 결과를 포함해 중복 적용 여부를 먼저 판정한다.
- recovery payload는 최소 `step_id`, `dedupe_token`, `reconcile_summary`, `existing_commit_sha`를 포함하고, agent는 이 값을 그대로 최종 JSON에 반사해 orchestrator가 duplicate apply를 차단할 수 있게 한다.
...
```json
{
  "step_id": "round3-step3-01",
  "dedupe_token": "round3:step3:round3-step3-01",
  "supervision": {
    "status": "thinking",
    ...
    "recovery_attempts": [
      {
        ...
        "completed_at": "...",
        "result": "success",
        "reconcile_summary": "worktree dirty, commit missing"
      }
    ]
  }
}
...
`step_id`는 `start_step_trace()` 시점에 한 번만 생성하고, 같은 step의 모든 retry/recreate 경로에서 재사용한다. `dedupe_token`은 이 `step_id`에서 결정적으로 파생해 state, recovery payload, agent final JSON에 동일 값으로 남긴다.
...
- [timing.py](/Users/jk/workspace/skills-jk/skills/cc-codex-debate-review/lib/debate_review/timing.py)
  - supervision summary schema와 `step_id`/`dedupe_token` persistence 추가
- [prompt.py](/Users/jk/workspace/skills-jk/skills/cc-codex-debate-review/lib/debate_review/prompt.py)
  - recovery prompt/message에 `step_id`, `dedupe_token`, `reconcile_summary`, `existing_commit_sha` 주입
...
- [cli.py](/Users/jk/workspace/skills-jk/skills/cc-codex-debate-review/lib/debate_review/cli.py)
  ...
  - mutation recovery 시 agent JSON의 `step_id`/`dedupe_token` 검증 및 mismatch fail-fast
...
- mutation 단계 retry/recreate에서도 동일 `step_id`/`dedupe_token`이 유지되는지
- recovery agent 응답의 `step_id`/`dedupe_token`이 state와 다르면 duplicate apply 전에 fail-fast 하는지
...
- step3가 commit 직전 hang된 뒤 recovery payload의 `dedupe_token`으로 duplicate apply 없이 수습되는 case

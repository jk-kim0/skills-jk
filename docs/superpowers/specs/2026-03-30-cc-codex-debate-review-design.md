...
- 전체 round loop를 실제로 실행하는 상위 orchestrator runner
- persistent agent create / resume / recovery의 런타임 자동화
- failure 이후 bug report 생성
- terminal 시 PR title/body 갱신
- worktree cleanup

즉, 이 문서의 설계는 여전히 유효하지만 현재 구현물은 "완성된 debate-review 시스템"이 아니라 "상태 엔진 + prompt assets + 운영 절차 문서"에 가깝다.
...
이 설계 문서의 역할 분담은 "최종 시스템" 기준이다.

- CLI는 상태 전이와 보조 계산을 담당한다.
- 상위 orchestrator는 round loop, agent dispatch, failure handling, cleanup을 담당한다.

현재 코드베이스에는 전자가 구현되어 있고, 후자는 아직 저장소 내부 실행 코드로 닫혀 있지 않다. 따라서 이 문서를 구현으로 옮길 때는 CLI 완성도만 보지 말고 orchestrator 경로까지 별도 완료 조건으로 관리해야 한다.
...
이 설계를 "기능 완성"으로 판단하려면 아래 항목이 모두 충족되어야 한다.

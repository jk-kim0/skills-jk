...
라운드 루프 (최대 10라운드, lead agent 교대)
  ...
  │          → lead agent가 PR을 리뷰하고 raw reports 생성
...
#### Step 1 기록 (finding이 있는 경우)

- `rounds[N].step1.report_ids` 기록 (L이 생성한 report ID 목록)
- `rounds[N].step1.issue_ids_touched` 기록 (L이 이번 round에 직접 보고한 issue ID)
- `rounds[N].step1.verdict = "has_findings"`
...
0. Step 3 시작 직전에 `journal.step=step3_lead_apply`, `journal.commit_sha=null`, `journal.push_verified=false` 로 초기화
1. `commit_sha`
2. `push_verified`
3. `state_persisted`

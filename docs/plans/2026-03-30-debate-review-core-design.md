...
- repo-owned orchestration path (`bin/run-debate-review`)
- persistent agent create / resume / recovery / supersede / terminal processing
- final comment 생성 / 게시, failure issue 생성, worktree cleanup
- runtime supervision / timing / reporting
...
남아 있는 일은 핵심 기능 구현이 아니라 문서/용어 정리와 dead-path cleanup에 가깝다.

현재 열린 cleanup backlog의 canonical 목록은 [2026-04-01-debate-review-persistent-agent-impl.md](./2026-04-01-debate-review-persistent-agent-impl.md)에서 관리한다. 이 문서 아래에 남아 있는 fork PR 관련 서술은 historical design note이며, 현재 지원 범위를 뜻하지 않는다.
...
- **Recommendation**: 제거된 fork 경로에서 쓰이던 historical application 상태. 현재 지원 범위에서는 생성되지 않아야 한다.
...
초기화
  ...
  │   └─ fork PR이면 즉시 종료
  ├─ target_ref 결정
  │   └─ same-repo PR에 대해 refs/debate-sync/pr-<number>/head 사용
  ├─ 상태 파일 로드 또는 초기화
  └─ Round 1 시작
...
**Historical note: 제거된 fork PR 경로**
- 과거 설계에는 `application_status=recommended` 기반 fork 경로가 있었지만, 현재 구현은 fork PR을 지원하지 않는다.
- fork 관련 나머지 필드와 전이 설명은 dead-path cleanup 대상 참고 정보다.

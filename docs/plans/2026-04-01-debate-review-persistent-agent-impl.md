...
- 기본값이 이미 `persistent`
- legacy 경로는 아직 남아 있음
- `settle-round`의 `debate_ledger` 정산 반영이 `#162`로 병합됨
- 다중 플래그 시 application phase sequential 실행 보장이 `#163`으로 병합됨
- duplicate withdrawal의 state-side 1차 반영이 `#164`로 병합됨
- Step 3 Phase 2의 short SHA 정규화가 `#165`로 병합됨
- round / step timing instrumentation이 `#166`으로 병합됨
...
기존 rollout 계획을 현재 상태에 맞춰 재해석하면, 남은 일은 아래 네 묶음이다. 이 목록이 debate-review completion의 canonical backlog다.

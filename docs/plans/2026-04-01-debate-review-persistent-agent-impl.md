...
1. debate-review를 end-to-end로 실행하는 orchestrator runner가 저장소 안에 존재한다.
2. 기본 모드인 `persistent` 경로가 최신 schema와 완전히 일치한다.
...
기존 rollout 계획을 현재 상태에 맞춰 재해석하면, 남은 일은 아래 다섯 묶음이다.
...
- short SHA 입력을 full SHA와 안정적으로 비교
- push verification 실패 후 resume 경로 검증
- phase1 / phase2 / phase3 중간 재시작 검증

### Workstream C: Orchestrator runner implementation

현재 가장 큰 공백이다. 저장소에는 state CLI는 있지만 round loop를 실행하는 코드가 없다.
...
1. persistent prompt / state routing parity 복구
2. application path hardening
3. orchestrator runner 구현

...
```
 ...
 ~/.claude/pr-review-state.json             ~/.codex/pr-review-state.json
         |                                           |
         v                                           v
   /code-review <PR URL>                   Codex 자체 리뷰 로직
         |                                           |
         v                                           v
...
| `/code-review` | `codex review` 또는 Codex 자체 리뷰 로직 | 이번 설계에서는 PR URL 기반 자동 리뷰를 위해 Codex 자체 리뷰 로직을 사용 |
...
```bash
env -u GITHUB_TOKEN -u GH_TOKEN gh search prs --review-requested=@me --state open --json number,repository,createdAt,url
env -u GITHUB_TOKEN -u GH_TOKEN gh pr list --repo <owner/repo> --state open --json number,createdAt,headRefOid,url
```
...
- 같은 에이전트가 같은 `head_sha`를 이미 성공적으로 리뷰했다면 skip
...
2. PR에 `[auto-review:claude][sha:<head_sha>]` 댓글이 실제로 존재함
...
```cron
*/5 * * * * PATH=/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin /opt/homebrew/bin/codex exec "Review pending PRs (agent=codex): load ~/workspace/skills-jk/skills/ops/pr-auto-review.md and execute it"
```
...
1. 리뷰 결과 생성 성공
2. PR에 `[auto-review:codex][sha:<head_sha>]` 댓글이 실제로 게시됨
...
- finding이 없으면 "No actionable findings" 같은 짧은 댓글 대신 skip 가능

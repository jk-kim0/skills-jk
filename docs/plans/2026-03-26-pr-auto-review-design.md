...
**날짜:** 2026-03-26
**상태:** 구현 완료 (config, skill 파일, cron 설정 완성 / `/ralph-loop` 세션 실행은 사용자 수동 시작)
...
- draft 제거: `isDraft=true` 항목을 cap 이전에 제거 (수집 단계에서 이미 확보된 필드)
- 정렬 기준: `createdAt` 오름차순
- 처리 상한: `max_prs_per_run` — draft 제거 후 후보 풀에 적용하며, 실제 리뷰 게시 수가 아니라 이후 상태/댓글 기반 skip까지 포함한 후보 수를 제한한다

예시 명령:

```bash
env -u GITHUB_TOKEN -u GH_TOKEN gh search prs --review-requested=@me --state open --limit 100 --json number,repository,createdAt,url,isDraft
env -u GITHUB_TOKEN -u GH_TOKEN gh pr list --repo <owner/repo> --state open --limit 100 --json number,createdAt,headRefOid,isDraft,url
```
...
```cron
*/5 * * * * PATH=/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin /opt/homebrew/bin/codex exec -s danger-full-access "Review pending PRs (agent=codex): load $HOME/workspace/skills-jk/skills/ops/pr-auto-review.md and execute it"
```

`-s danger-full-access`: 상태 파일(`~/.codex/pr-review-state.json`)이 workspace 밖에 있으므로 전체 파일시스템 쓰기 접근 필요. cron은 `codex exec`의 비대화형 모드로 실행되며 승인 프롬프트가 없다.
...
- 리뷰는 `gh pr diff` 출력만을 입력으로 사용한다. 대상 repo의 전체 파일 트리, 호출부/정의부, AGENTS.md 등 diff 바깥의 컨텍스트는 포함되지 않는다. 이는 외부 repo checkout 없이 동작하기 위한 의도된 트레이드오프다.

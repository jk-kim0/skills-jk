...
- Run `bin/hermes-sync-env` only when secrets change. This materializes a local Git-ignored `.hermes/.env` file and is the step that may require 1Password authorization.
- Run Hermes from the local `.hermes/.env` file so normal reads do not call 1Password: `HERMES_HOME="$PWD/.hermes" bash -lc 'set -a; source .hermes/.env; set +a; hermes'`.

...
5. Restart/redeploy the affected runtime.
   ...
   - For Tencent VM deployments driven by GitHub Actions, prefer the repository workflow path over ad-hoc local SSH: use the workflow's explicit target selector, set `update_gmail_oauth_config=true`, deploy a known-good image, then verify the target jobs and public `/login` health. See `references/tencent-vm-gmail-oauth-deploy-verification.md`.
6. Run staged smoke.
...
- When intentionally testing a bad Gmail OAuth client secret on VM deployments, use the same secret-sync and restart path as a real repair, verify the product surfaces the token-exchange failure reason, then restore the correct deployed OAuth client pair from the secret store and redeploy. Record only workflow/run evidence and safe metadata, never secret values or raw env files. For outbound-agent Tencent VMs, see `outbound-agent-dev-environment-operations` reference `references/tencent-gmail-oauth-negative-test-and-restore.md`.
- Google may omit `refresh_token` on repeated consent for an already-approved OAuth client/scope. If the app has no existing stored sender credential, this is a distinct recovery path from account mismatch: verify selected sender state and Google `id_token.email` first, then advise revoking app consent in Google Account access and retrying from the intended sender row. In resettable dev environments, check whether a recent DB reset/seed removed the stored credential while recreating the sender row as `auth_required`; if so, OAuth config can be correct even though callback fails.
- Do not let app code collapse Google token endpoint failures into `refresh_token_missing`. `!response.ok` should preserve a safe provider error/status such as `invalid_client`, `invalid_grant`, or `redirect_uri_mismatch`; only `response.ok` with no refresh token and no reusable DB credential should use the refresh-token-missing path.
- In multi-sender products, prefer row-level Gmail connect actions with signed/validated sender identity in OAuth state. A single global connect action makes wrong-row/wrong-account failures harder to diagnose; callback code should fail account mismatch before refresh-token-missing handling.
...
- `references/gmail-refresh-token-missing-diagnostics.md` — Sender-scoped Gmail OAuth diagnostics for missing refresh tokens versus wrong-account/missing-state errors, with safe evidence fields and UX guidance.
- `references/tencent-vm-gmail-oauth-deploy-verification.md` — Tencent VM GitHub Actions pattern for syncing Gmail OAuth env into VM-local env files, deploying/restarting known images, and verifying without exposing secrets.

...
- When the user asks to apply a config key to all profiles, treat it as a bulk profile-as-code change: update the main config plus every checked-in `profiles/<name>/config.yaml` under the active `HERMES_HOME`/repo-managed `.hermes` tree. Prefer a deterministic YAML patch/update loop over manually running `hermes config set` one profile at a time, preserve unrelated formatting when possible, and verify every target file has the requested value. Do not touch profile runtime state such as sessions, logs, auth DBs, cron output, or `.env` files.
...
- If the skills index is the remaining large bucket, consider a `skills.index_mode` design (`full | compact | lazy | off`) rather than only moving skills between profiles. See `references/skills-lazy-index-design.md` for a concrete implementation plan and pitfalls.

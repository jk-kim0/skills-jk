...
- Next.js typed routes can reject dynamic `Link href` values built from runtime strings with errors like `Type 'string' is not assignable to type 'UrlObject | RouteImpl<string>'`. If the route pattern is intentional and already covered by the App Router tree, prefer importing `type { Route } from "next"`, assigning the computed href once (for example `const homeHref = (...) as Route`), and reusing that typed value in `Link`. Avoid scattering inline casts across JSX; it makes future route audits harder.

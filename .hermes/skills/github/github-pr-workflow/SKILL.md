...
9a. Manual deployed-URL E2E workflow, Node runtime upgrades, and WAF-aware throttling.
   ...
   - If the merged E2E spec has safety guards that reject production/stage databases or non-local hosts, do not wire the manual workflow directly to live development URLs just because the user says “three dev servers.” Instead, preserve the safety boundary by running the app against an ephemeral Actions-local database and local Next/dev server, and expose the requested server choices as isolated local dev-server slots when that matches the test contract. Make this explicit in the PR body so reviewers understand that the workflow validates the merged local-only E2E safely rather than mutating shared dev environments.
   - When the user requests a naming prefix such as `E2E - ` for workflow `name` attributes, apply it consistently to the workflow display `name`, `run-name`, job `name`, and named steps/artifacts where practical; do not only update the top-level workflow name.

...
- When a wiki gap/audit page is used as the source for implementation work, close the loop after the PR is pushed: add a short `Follow-up status`/`후속 조치 현황` section to the wiki page with the PR link, CI status if known, what was addressed, and what remains. Do not rewrite the original audit conclusions as if the PR were already merged; keep them as historical findings and append the follow-up status.
- Before creating a new PR for wiki-driven follow-up work, check whether a branch/PR for the same topic already exists and update that PR instead of opening a duplicate.
...
- When starting from a wiki gap/status page and the user asks to resolve remaining work, treat the wiki as a live tracker rather than only a prose source: pull the wiki first, respect any remote page renames (for example directory-style wiki files may have been flattened to hyphenated slugs), then compare the page against latest `origin/main` and currently open PRs. If an open PR already covers the recommended fix, update that PR instead of creating a duplicate, and update the wiki wording to distinguish `main` status from "in progress in PR #..." status.
- When a wiki gap page lists implementation gaps, refresh stale resolved items after code has advanced: include the latest base commit, replace outdated file paths/routes, mark resolved UI/route items as implemented or partial, and leave only genuinely remaining gaps as action items. Avoid letting wiki tracker pages continue to claim a missing route/button/feature after latest main added it.
- When the user asks to rewrite a wiki gap page against latest `main`, create or refresh a detached worktree from `origin/main`, read any repo-maintained feature/status document first if one exists (for example `docs/feature/status-*.md`), then verify representative schema/service/UI/test files. Use that status doc as a summary input, not as the only source of truth. Rewrite the wiki around current conclusions, a resolved-items section for old gaps, and a concise remaining-gap table rather than preserving outdated long-form audit prose.
- For gap/status wiki rewrites, always update evidence links to the exact latest main commit SHA and verify the wiki push with `git status`, `git log -1`, and `git ls-remote`. This prevents stale line links and confirms the `.wiki.git` repository actually advanced.
- For full rewrites of gap tracker wiki pages against latest `main`, do not preserve the old blocker narrative just because it is detailed. Re-audit the implementation from a fresh `origin/main` worktree, classify old findings into `Resolved gap` and `Remaining gap`, and make the conclusion reflect current code reality. Repo docs/status pages are useful evidence but can have stale `basis commit` metadata; when they conflict, prefer latest `origin/main` source and explicitly call out documentation drift as a remaining gap rather than copying stale status wording.
...
- GitHub Wiki page names should be treated as flat slugs. Do not create normal wiki pages under subdirectories such as `gap/gmail.md` or link to page names containing `/` such as `[gap/gmail](gap/gmail)`; GitHub's wiki web routing can interpret the slash as an extra path segment and the link may 404/misroute. Use a top-level delimiter such as `gap-gmail.md`, `deployment-vercel.md`, etc., and link to `[gap-gmail](gap-gmail)`.
- When a wiki needs grouped child pages, create the parent/category page as a top-level slug file such as `gap.md` or `deployment.md`, and create child pages as top-level hyphenated slugs such as `gap-gmail.md` or `deployment-vercel.md`. Keep the parent page short: describe the class of child documents and include simple links to each child page. If `Home.md` currently links directly to every child under that group, update it to link to the new parent page instead, unless the user asked to keep the flat home index.

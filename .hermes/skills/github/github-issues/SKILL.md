...
metadata:
  hermes:
    ...
    related_skills: [github-auth, github-pr-workflow, openspec-decision-management]
---
...
- Before any `gh` call, check active repo/global agent guidance for required invocation wrappers. Some user environments require every GitHub CLI call to be run as `env -u GITHUB_TOKEN gh ...`, including read-only issue/PR/run lookups. If such guidance is present, apply it to all examples below rather than using raw `gh ...`.
...
- Practical `gh issue edit --body-file` pitfall: when editing from a detached worktree or alternate cwd, ensure the body file path is resolvable from that cwd. Prefer an absolute path. For issue-only edits that should not create repository changes, write the draft under `/tmp` (for example `/tmp/<repo>-issue-<n>-body.md`) rather than under `.agents/`, `.hermes/`, or the repo root; verify `git status --short --branch` is still clean before editing the issue.
...
- For meeting-style review issues (for example OKR review, blocker review, roadmap review), use the issue body as the pre-read / meeting material and then add the actual meeting notes, decisions, deferred decisions, and action plan as a follow-up issue comment after discussion. Do not silently rewrite the original pre-read into final conclusions unless the user explicitly asks to convert the issue body into the canonical current-state record.
...
- For a reusable step-by-step pattern to refresh a living audit issue from latest `origin/main`, see `references/living-audit-issue-refresh.md`.
...
- For DB/deploy/runtime tracking issues after an implementation PR has merged, separate `main implementation reflected` from `operational evidence still pending`. Record the latest `origin/main` SHA, the merged PR URL/merge commit, concrete artifacts now present on main (migration/backfill/workflow/runtime-log code), and the missing evidence still required (for example shared DB migration/backfill run logs, dev/prod workflow run URLs, runtime smoke/log markers). Keep the issue open when code is merged but environment execution evidence has not been captured.
- If the user explicitly asks to close resolved open issues while refreshing from latest `main`, actively classify each issue as: resolved on `origin/main` (close), blocked by an open PR (keep open and mention the PR), or still missing concrete spec/work (keep open and list close conditions). Before closing, comment with the evidence and why it is complete; then close with `--reason completed`. Do not close issues that are only resolved on an unmerged PR.
- Practical `gh issue close` pitfall: some `gh` versions do not support `--comment-file`. If a closing note is more than a short shell-safe string, first run `gh issue comment <n> --body-file <path>`, then `gh issue close <n> --reason completed`.
...
- When updating multiple open issues from current `main` and open PR state, treat repository state as moving until the final verification pass. After editing the issue bodies, re-run `git fetch origin main --prune`, `gh pr list --state open`, and any relevant CI/run checks before finalizing. If a PR merges, a new PR appears, or a workflow/check transitions state mid-session, immediately reclassify the affected issue bodies and edit them again rather than leaving "no open PR", "CI in progress", or other now-stale status text in the body.
- For any living issue refresh where the body records baseline SHA, open PRs, or CI status, do a post-edit verification read of the published issue plus a fresh PR/run check. If that final check reveals state drift that happened during the edit window (for example a newly opened related PR, a just-merged PR, or an in-progress main run completing), perform a small second edit immediately so the real GitHub issue is not stale at the moment you report completion. Verify the final body with boolean checks for the newly added PR/status text and absence of superseded phrases such as `관련 open PR: 없음.`, not just the returned issue URL.
- Practical final-pass pattern for fast-moving repos: after `gh issue edit --body-file`, save `gh issue view ... --json title,body,updatedAt,url` to a temp file, run boolean body checks for the baseline SHA and related PR/status strings, then run `gh pr list` and `gh run list` one more time. If a new open PR appears after the first edit, patch the temp body file and run `gh issue edit` a second time before the final reply.
- When the final `gh pr list` shows newly opened PRs after the first issue edit, do not dismiss them just because they were absent from the initial scan. Quickly classify whether each one is in-scope, adjacent docs/status-only, or unrelated; if in-scope or adjacent to the issue's tracked feature/status, update the issue's related-open-PR section immediately and verify the second edit. This prevents a just-refreshed living issue from being stale at the moment it is reported complete.
- When the final `git fetch origin main --prune` shows `main...origin/main [behind N]` after an issue refresh, do not report the just-edited issue as current. Fast-forward local `main`, inspect the newly merged commit(s) in the issue scope, reclassify any open PR references that just merged, update the baseline SHA and CI/run status, and edit the issue again. If CI status changes during this loop (for example in-progress becomes success), perform a small final status edit rather than leaving stale wording.
- For multi-issue refreshes in fast-moving repos, repeat the final drift loop for every edited issue, not just the last one. If `origin/main` advances because an adjacent PR merged during the edit window, fast-forward, inspect the new commit's changed files against each issue's scope, and re-edit all affected issue bodies so they share the same latest baseline. If a newly opened adjacent PR appears after the edit, add it to every affected issue rather than leaving one issue current and another stale.
- For volatile PR/check status that flips between `BLOCKED`, `UNSTABLE`, queued, and in-progress while you are editing, avoid chasing exact transient merge-state wording through many edits. Record the durable classification plus a truthful bounded phrase such as "checks in progress / merge state 변동 중" unless the final exact state is stable and directly material to the issue. Still verify the body contains the related PR and latest baseline SHA before reporting completion.
- When the final `git fetch origin main --prune` shows `main...origin/main [behind N]` after an issue refresh, do not report the just-edited issue as current. Fast-forward local `main`, inspect the newly merged commit(s) in the issue scope, reclassify any open PR references that just merged, update the baseline SHA and CI/run status, and edit the issue again. If CI status changes during this loop (for example in-progress becomes success), perform a small final status edit rather than leaving stale wording.
...
- For preview/static page taxonomy issues after route promotions, reclassify by current route inventory, not historical route names: enumerate remaining preview routes directly from `origin/main`, enumerate promoted public routes separately, and narrow remaining work to the surviving families (for example “plans preview” or “ACP child preview”) rather than preserving stale broad `/t/*` or primitive-commonization TODOs. If there are no open PRs, state that explicitly instead of leaving old open-PR references in the body.
...
- When refreshing a living audit issue after multiple follow-up PRs have merged, treat the update as a full reclassification from latest `origin/main`: list the post-baseline commits/PRs touching the issue scope, inspect the current source/test/doc inventory, check related open PRs, and convert stale broad implementation TODOs into a reference/audit status when no immediate code change remains. If the user prefers avoiding local verification, it is acceptable to cite latest relevant GitHub Actions runs plus static source scans, but the issue body must explicitly say local build/test was not rerun.
- When the user asks to “do remaining work from issue #N” on a living blocker/status issue, first re-fetch latest `origin/main`, read the current issue body/comments, recent commits, and open PRs, then choose only a concrete non-overlapping item that is already unblocked by documented decisions. Also inspect same-topic local worktrees/branches before creating a new branch: if a local branch has no open PR and rebasing it onto `origin/main` skips its commit as already applied, treat the old branch as evidence that the work already landed (find the merged PR/commit) rather than reviving it. Reclassify the issue from latest main, then implement only the still-current remainder. If a remaining item still requires a product/route/policy decision, do not implement around it by guessing; explicitly leave it out of scope and pick a smaller implementable hardening/API/UI item instead. After opening the PR, add a neutral issue comment that states exactly which remaining-work bullet was advanced, which prior PR/commit already resolved stale bullets, and which decision-bound items were intentionally not changed.
- When the user asks whether blockers derived from a living issue are implementable or blocked by decisions/external information, treat it as a current-state triage rather than a planning-only answer. Fetch latest `origin/main`, re-read the issue body/comments, check related PRs that the issue body names as open or pending (they may have merged since the issue was last edited), inspect the canonical decision/spec documents for `Status: Accepted` vs `Open/TBD`, and read the relevant implementation files just enough to confirm current gaps. Classify each blocker into: (1) now implementable because decisions have landed, (2) still blocked by Product Owner decision, (3) still blocked by external credentials/provider/ops setup, or (4) already resolved/stale in the issue body. If an issue body is stale because named PRs merged, say so explicitly before recommending implementation scope.
...
- When a living route/technical-debt issue previously centered on preview-route promotion, explicitly re-count the current preview route tree before preserving any promotion TODO. If `src/app/t/**/page.tsx` is now empty or the relevant family has moved to public routes, remove the stale preview-promotion work, reclassify merged PRs as completed, and reframe remaining work around post-promotion debt such as redirect/compatibility handlers, internal/demo surfaces, source-coupled tests, and stale docs/skills/plans references.
...
- When multiple main-branch runs exist for the same head SHA, distinguish them by workflow/job family instead of collapsing them into one “CI/build” status. For example, record `CI: success`, `Deploy outbound-dev Production: success`, and a separate cache/build/deploy run as `queued` / `in_progress` / `success`. If one workflow completes while another is still moving, update the issue with this split status rather than saying the whole head is either green or queued.
- If a workflow state changes during the issue-edit window, immediately do a small second edit rather than leaving the just-refreshed body stale. Common case: an apply/migration run that was `in_progress` when the first body was drafted completes as success/failure before final verification. Re-read the run after `gh issue edit`, replace stale `in progress` wording with the final result, and verify the body no longer contains the superseded phrase.
- For multi-issue refreshes, apply that second-edit loop to every affected issue, not just the issue whose body you happened to edit last. If final `gh run list --branch main` shows CI/deploy runs moved from `in_progress` to `success` after the first edit, rewrite the affected issue bodies to record the new success while preserving any still-running runs separately, then verify with boolean body checks for both the new success phrase and any remaining `in_progress` phrase that is still true.
- When a living issue tracks paired operational workflows, distinguish them instead of collapsing them into one DB/CI gate. For example, after a mutating `Apply ... Migration` run succeeds, a standalone read-only `Check ... Schema` workflow may still be queued or in progress; record the apply success and the standalone check's current run ID/status separately, then leave only the unfinished standalone check as the remaining gate.
- When a living issue refresh depends on an open related PR, verify that PR's current `mergeStateStatus`, head SHA, and `statusCheckRollup` before editing; after editing the issue, do one quick re-read of the PR status to catch fast-moving CI changes. If a build/check is still in progress, leave the issue truthful with the in-progress job named and do not wait passively for completion unless the user explicitly asks.
...
- If the user challenges why an issue classifies an intentional route/surface/workflow as technical debt, do not defend the old wording by default. Reframe the issue around the real debt boundary: the intentional behavior itself is not debt; any remaining debt must be a concrete maintainability gap such as missing owner, lifecycle/removal criteria, scattered contract documentation, or risk that future agents/PRs will misclassify it. Update the issue title/section, remaining-work list, and done criteria so they no longer imply removal of the intentional surface.
...
- When the user asks to convert an existing issue into a repository guide or durable documentation, do the real repo workflow rather than only rewriting the issue:
  ...
  5. after opening the PR, comment on the issue with the PR URL and a short description so the issue remains navigable, unless the PR body intentionally uses a closing keyword and the user only asked for merge-time closure
...
- When the user says they will make or manage a decision from an existing issue in a repo that has a canonical decision-log system (for example OpenSpec `changes/<change-id>/design.md`, ADRs, or similar), treat it as a real documentation workflow, not just a chat answer or issue edit:
  1. read the issue and the repo's decision-log guidance
  2. create/update the decision-log document with the question, current status, options, schema/code evidence if relevant, decision criteria, implementation impact, and a clear recording rule for when the decision becomes accepted
  3. keep the decision `Open` / `TBD` unless the user explicitly provided the final policy choice
  4. add follow-up checklist items in the relevant tasks/plan doc when the accepted decision must later propagate into specs, user-facing scenarios, or implementation contracts
  5. open a docs-only PR and comment on the originating issue with the PR link so the issue remains navigable
  6. avoid auto-closing keywords unless explicitly requested
- For issue-to-doc conversions where the new document is meant to replace the issue as the canonical audit/reference artifact:
  1. re-audit the latest `origin/main` instead of copying stale issue counts or TODOs verbatim; explicitly incorporate any recently merged PRs that changed the tracked inventory
  2. structure the document around current status, quick verification commands, expected current results, decision/status tables, and a "how to update this document" section
  3. when the issue tracks links/URLs, include exact scan scope and exclusion rules so future counts are comparable, then separate categories that look similar but have different treatment (for example owned-host migration targets vs docs/app/trust subdomains)
  4. if an adjacent or similarly named worktree/branch already exists for another issue or audit, inspect it but do not reuse it when it has staged/different issue scope; create a distinct fresh worktree/branch so the PR stays issue-specific
  5. if `origin/main` advances during the conversion—especially because a related PR merged while you were drafting or just before push—rebase, inspect the newly merged files in the issue scope, update the new document's baseline SHA/status counts/gap classifications, and amend before opening or updating the PR. Do not leave the document pinned to the pre-merge state when the PR is based on a newer main.
  6. if the user explicitly says the PR should replace/close the issue on merge, include the appropriate auto-closing keyword in the PR body (for example `Closes #521`) and verify the created PR body actually contains it
  7. after PR creation, verify the remote branch SHA, PR `headRefOid`, and attached checks/runs; for docs-only PRs report in-progress checks without passively waiting unless asked
- For issue-to-doc conversions based on an audit issue, preserve the audit boundary explicitly in the new document. Include: latest baseline SHA/time, included/excluded paths, whether the scan is static-only or live-validated, owned-host vs third-party classification, current summary counts, high-priority review tables, known false positives, and a "how to refresh this document" section with the exact scan scope/commands. Do not copy the issue verbatim; convert it into a maintainable reference that future contributors can re-run and update.
- If the user explicitly asks that the issue close when the docs PR merges, put the closing keyword in the PR body (for example `Closes #522`) and mention that in the issue comment. This is an allowed exception to the default non-closing issue-reference rule.
...
### Open issue completion audit

When the user asks to inspect open issues and decide whether they are completed, treat it as an evidence-based audit from the latest main state, not as a generic issue list summary.

1. Fetch the latest `origin/main` and capture the SHA used as the audit baseline.
2. List open issues and open PRs. For each open issue, classify it as:
   - `resolved on origin/main` — all completion criteria are satisfied by merged commits or successful production/manual workflow runs.
   - `covered by open PR` — likely resolved only if an open PR merges; keep the issue open and name the PR/check status.
   - `partially complete` — some checklist items are done but concrete remaining work is still visible.
   - `still open` — no merged/current evidence satisfies the core request.
3. Inspect merged PRs and first-parent main history for issue numbers, related titles, and changed files; do not rely only on issue body checkboxes, which may be stale.
4. For deployment/DB/runtime issues, use GitHub Actions evidence when local verification is intentionally avoided: workflow run status, head SHA, job logs, schema drift checks, migration/backfill logs, and deploy checks. Quote the exact run IDs/URLs or commit SHAs that support the classification.
5. If browser/curl live smoke is blocked by Vercel Authentication, Deployment Protection, or another access control layer, state that limitation explicitly and do not overclaim app-level runtime verification. Use available CI/workflow/schema evidence and mark the protected smoke as not directly reproduced.
6. For DB migration / deployment issues, distinguish the issue's actual close criteria before classifying it: an issue scoped to producing migration artifacts, runbooks, and manual workflows can be a close candidate once those are merged and main/production checks are green, even if a protected PR Preview smoke could not be reproduced directly. Conversely, an issue scoped to automated runtime smoke/log observability should stay open when only manual smoke instructions exist.
7. Treat issue-body checkboxes as potentially stale. Reclassify from merged PR bodies/files, current `origin/main` inventory, current workflow files, and recent GitHub Actions runs; then call out when the issue body should be refreshed before or while closing.
8. Do not close issues unless the user explicitly asks to close them. It is fine to recommend `close candidate` with the evidence and a suggested closing comment.
9. If the audit produces a follow-up PR or documentation/skill PR that resolves only one slice of an issue, comment on the issue with the PR URL and the exact slice it addresses, then re-check the PR `headRefOid`, `mergeStateStatus`, and attached checks/runs. Keep the issue classified as open/partial until the remaining close criteria are merged and verified; do not treat the newly opened PR as closure evidence.
10. For partially complete long-lived issues, recommend a body refresh that separates `completed on main` from `remaining work`, instead of preserving stale broad checklists.

...
- When a user challenges an issue's remaining-work classification, re-evaluate the underlying design model before defending the old finding. For responsive UI audit issues, do not preserve a one-page follow-up just because a measured width looks smaller; first decide whether the component is intentionally fluid (for example an equal-grid card gallery that divides available width into 1/2/3 columns with fixed-height rhythm). If the observed delta is actually a shared policy difference across multiple pages (for example live gutter 24px vs local primitive gutter 30px), rewrite it as a policy/non-issue note rather than a page-specific defect.

...
- If the user asks to "write issue #N in Korean" (or another language) and the issue already exists, default to editing the real existing issue into that requested language rather than drafting new text only. After editing, immediately verify with `gh issue view N --json title,body,url` and return the URL.

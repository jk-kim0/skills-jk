...
- For issue-to-doc conversions where the new document is meant to replace the issue as the canonical audit/reference artifact:
  ...
  5. if the user explicitly says the PR should replace/close the issue on merge, include the appropriate auto-closing keyword in the PR body (for example `Closes #521`) and verify the created PR body actually contains it
  6. after PR creation, verify the remote branch SHA, PR `headRefOid`, and attached checks/runs; for docs-only PRs report in-progress checks without passively waiting unless asked

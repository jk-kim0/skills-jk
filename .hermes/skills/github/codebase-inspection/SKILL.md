...
10. When asked whether a feature plan can move to a done/archive area, audit the plan's own acceptance criteria against the current implementation first. If complete, move the plan to the repo's done/archive location (for example `docs/done/`), mark it `Done`, add concise completion evidence, search for stale references to the old path, and update both the feature index and aggregate status/count rows so no stale `In-Progress` pointer remains.
11. If an aggregate status document already contains detailed rows for the same feature, avoid maintaining two competing sources of detail. Move or keep the detailed inventory in the per-feature status document, and replace aggregate rows with concise links/summaries that point to it.
12. Report concise evidence with exact paths and the current behavior. Avoid overstating capability: code for an adapter is not the same as a complete user-facing production flow.

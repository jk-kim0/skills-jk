...
- Repo-local Hermes profile settings are also repo-managed when intentional: track `.hermes/profiles/<profile>/config.yaml` and `.hermes/profiles/<profile>/SOUL.md`, but exclude profile `.env`, sessions, logs, cron output, auth/state DBs, and other runtime files. If profile directories are missing in the root checkout but exist in a PR worktree, mirror the durable files back into root as the live local profile state instead of assuming the profile setup is complete.
- After profile-as-code lands on `origin/main`, do not cleanup with `rm -rf .hermes/profiles`; that deletes tracked profile files. Use `git restore -- .hermes/profiles` for tracked residue and `git clean -fd -- .hermes/profiles` only for untracked runtime files, then re-check `git status --short --branch`.
- Conversely, do not over-exclude tracked `.hermes/` changes just because they look local. Durable repo-managed updates such as `.hermes/memories/MEMORY.md`, bundled skill content under `.hermes/skills/**`, derived `.hermes/skills/.bundled_manifest`, intentional `.hermes/config.yaml` updates, and intentional profile `config.yaml`/`SOUL.md` updates can all be legitimate PR payload when the user asks to turn the current local Hermes workspace sweep into a reviewable PR.
...
- **create-pr.yml 자체 업그레이드 주의**: `.github/workflows/create-pr.yml`의 action runtime을 업그레이드하는 PR을 이 workflow로 생성하면, PR 생성 run 자체는 default branch의 기존 `create-pr.yml`을 사용하므로 Node runtime deprecation warning이 한 번 더 보일 수 있다. 이것을 branch 실패로 보지 말고, PR diff와 candidate action tag의 `runs.using: node24`를 검증한 뒤 merge 이후 future dispatch부터 해결된다고 설명한다. 세부 절차는 `references/create-pr-workflow-node-runtime-upgrade.md` 참고.
...
- Treat profile-local runtime/install state such as `.hermes/profiles/<profile>/.update_check`, `.hermes/profiles/<profile>/lsp/**`, and `.hermes/profiles/<profile>/skills/**` as cleanup residue by default, even when durable profile files (`config.yaml`, `SOUL.md`) are repo-managed. Exclude those runtime directories from PR payloads and remove them with targeted `git clean -fd -- <paths>` during final workspace cleanup.
...
- Re-run root status after each PR/worktree creation. New tracked skill/reference edits can appear during the session after final verification; if they are meaningful and not part of the just-created PR, split them into another narrow PR rather than silently leaving them unreported.
- Re-run root status again after restoring the pushed payload and fast-forwarding root `main` to `origin/main`. A just-merged prior follow-up PR can make root clean enough to fast-forward, but that fast-forward may reveal additional same-scope skill/reference residue created during the current session. If a same-scope PR is still open, copy that newly revealed residue into that PR worktree, amend/force-push, update the PR body, then restore/clean root again before final reporting.
- When several follow-up PRs are open in the same sweep, compare their actual `origin/main...HEAD` payloads before final reporting. If a support file or skill edit was accidentally included in the wrong/broader PR, amend that PR to remove the duplicate and keep the file only in the narrow intended PR.
...
- See `references/behind-root-stale-deletion-hunks.md` for cases where copying tracked skill files from a dirty root checkout that is behind `origin/main` creates stale deletion hunks in the fresh PR worktree; preserve latest-main guidance and keep only the real local additions.
...
- if root `main` is behind `origin/main`, copying dirty tracked skill files into a fresh latest-main worktree can produce stale deletion hunks that would remove newly merged guidance. Before staging, inspect diffs for unexpected deletions and restore the latest-main lines so the PR contains only the real local additions. See `references/behind-root-stale-deletion-hunks.md`.
...
- If latest `origin/main` has moved repo-specific skills into `.hermes/skill-packs/<repo>/`, do not preserve stale root changes by re-adding the old `.hermes/skills/software-development/<repo>/` directory. Transplant the meaningful note/reference into the current pack path and update the pack skill index instead, so the PR does not undo the skill-pack split.
...
- if a newly revealed residue bucket appears after the first skill-residue PR is already opened, update that same PR when it belongs to the same cleanup scope: apply the delta to the PR worktree, amend the commit, force-with-lease push, update the PR body, restore the root residue, and loop root status again before fast-forwarding main. See `references/iterative-root-residue-amend-pr.md`.
- during root cleanup, a single dirty checkout can contain multiple unrelated `.hermes/skills/**` buckets: existing open-PR updates, new coherent skill-note PRs, newly revealed residue, stale deletion hunks from a behind root, and merged worktree residue. Re-run root status after each PR/update and split remaining meaningful residue into narrow PRs before resetting root. See `references/multi-bucket-root-residue-pr-split.md`.
- when the user explicitly names `.hermes/config.yaml`, `.hermes/memories/MEMORY.md`, and `.hermes/memories/USER.md` but those files are identical to latest `origin/main`, do not create or describe a scoped config/memory PR. If the same request also asks to inspect local changes broadly and meaningful `.hermes/skills/**` SKILL/reference residue survives, create a separate narrow skill-residue PR and make the no-op scoped-files fact explicit in the PR body and final report. See `references/scoped-config-memory-noop-skill-residue-pr.md`.
- in repeated cleanup turns, the requested scoped set can be partially non-empty while a stale local-only `preserve/*` branch also carries broad historical residue. Create the narrow scoped PR for the real requested diff, then inspect the preserve branch separately and promote only meaningful latest-main-portable subsets into one or more fresh PRs; never PR the stale preserve branch directly. See `references/repeated-scoped-memory-cleanup-with-preserve-branch.md`.
- if the requested scoped set has a real memory/config diff while an existing open skill-residue PR also exists, split the work: create a narrow scoped PR for the changed memory/config file(s), then amend the existing skill-residue PR with unrelated `.hermes/skills/**` changes. Re-run root status after each restore because additional untracked skill directories can be revealed. See `references/scoped-memory-pr-with-open-skill-residue.md`.
- if a scoped memory/config PR merges while you are still trying to push follow-up commits or edit its PR body, stop updating the old branch, create a fresh latest-main branch for the remaining diff, and report the merged PR and new PR separately. See `references/merged-pr-during-scoped-memory-update.md`.
- if a cleanup PR branch is re-pushed after its PR has already merged, GitHub may recreate the remote branch while the PR object remains `MERGED` with stale head metadata; treat this as a merge race, delete the accidentally recreated branch after creating a replacement PR for the remaining diff, and do a final generated-residue cleanup loop before declaring root clean. See `references/final-cleanup-merge-race-and-regenerated-residue.md`.
- if the requested scoped config/memory payload is already merged/no-op but a separate no-open-PR worktree has a focused dirty source/test patch, do not delete it as stale during cleanup. Commit it in that worktree, rebase onto latest `origin/main`, push it, and create a bot PR before root reset/cleanup. See `references/local-sweep-meaningful-nonpr-worktree-pr.md`.
- if terminal commands begin failing with `No space left on device` while creating Hermes temp/snapshot files, pause cleanup and check disk with `df -h` before further git/gh operations; remove only safe temp artifacts (for example oversized Hermes result files) and report the low-disk condition, because git hooks/tool snapshots may become unreliable even when the requested repo state is clean.

...
- when a scoped memory/config PR is created from dirty root `main` and the remaining root dirt is broad generated `.hermes/skills/**` residue, preserve the scoped payload on a pushed PR branch first, verify the remote head and PR file list, then reset root `main` to `origin/main` and delete stale merged local branches. Before copying scoped files, distinguish true local dirt from remote advancement with direct `origin/main:<path>` comparisons and `HEAD..origin/main` diffs. See `references/scoped-memory-pr-root-reset-cleanup.md`.

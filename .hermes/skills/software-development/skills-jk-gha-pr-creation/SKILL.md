...
- Re-run root status after each PR/worktree creation, and again after the first commit/push but before dispatching `create-pr.yml`. New tracked skill/reference edits can appear during the session after staging or committing (for example from loaded skill updates or residue revealed by the first commit). If they belong to the same cleanup-guidance scope and no PR exists yet, amend the branch before PR creation so the bot PR opens with the complete payload. If they are meaningful but outside the branch scope, split them into another narrow PR rather than silently leaving them unreported.

...
- When updating an existing `skills-jk` PR branch, re-check the PR body after adding commits. If the body says the branch was squashed to a single latest-main-based commit, either keep that invariant by soft-resetting/squashing again before the final push, or edit the body so it no longer claims a single-commit branch. Do not leave PR body verification claims stale after adding follow-up commits.

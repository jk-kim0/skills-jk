...
- If the root dirty set splits into several unrelated meaningful buckets, do not reset `main` after the first PR succeeds. Create/update as many narrow latest-main PR branches as needed, remove stale deletion hunks caused by the behind root base, verify every branch's remote SHA, then refresh open PR/worktree state again before deleting merged residue.

...
- After cleaning the root checkout with targeted `git clean` or `git reset --hard origin/main`, immediately run `git status --short --branch` again. A first clean/reset or same-session skill loading can reveal another authored tracked/untracked file under a different umbrella skill/reference path. Inspect it, add a SKILL.md pointer if needed, and if it is the same scope as an existing open preservation/refactor PR, apply the root diff into that PR worktree, amend/force-push, then reset/clean root again. Do not create a duplicate PR for same-scope follow-up residue.
- Treat runtime residue such as `.hermes/lsp/` or `.hermes/pairing/` separately from authored skill/reference files. Preserve authored files in a PR, then remove runtime residue from root.

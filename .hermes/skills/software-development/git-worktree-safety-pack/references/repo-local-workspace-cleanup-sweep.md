...
- If dirty root contains multiple candidate reference files for the same operations incident, do not preserve the one-session shape. Read the candidates, keep the topic-level filename/reference as canonical, fold unique durable details from the narrower duplicate into it, update the owning `SKILL.md` to point only at the canonical reference, and stage only that deduped payload.
...
- If a temporary PR body file is created under repo-local agent directories such as `.agents/tmp/` or `.hermes/tmp/`, delete it immediately after PR creation so the preservation worktree remains clean before the final sweep.

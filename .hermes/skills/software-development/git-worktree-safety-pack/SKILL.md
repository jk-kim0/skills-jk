...
22. Do not treat an empty broad open-PR list as the end of cleanup. Still run targeted PR lookup for every local branch and inspect `git branch -vv` upstream state. A branch can be a pushed PR-less preservation branch, a local branch whose PR just merged, a branch whose remote was pruned, or a clean worktree tracking `origin/main`; preserve PR-less meaningful commits by opening a PR, and delete clean merged/no-upstream/equivalent residue only after the targeted lookup and dirty sweep.

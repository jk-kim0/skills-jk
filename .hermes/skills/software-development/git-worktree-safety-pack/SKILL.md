...
- `references/pr-number-named-duplicate-and-obsolete-dirty-worktrees.md` — when local `work/pr*` branches are not the actual GitHub PR head branch, query the PR number directly and compare against the real head; also covers skipping dirty residue that only revives files intentionally deleted by latest `main`.
...
17. Dirty residue is not always meaningful current work. If a preservation commit conflicts on rebase because latest `main` deleted the modified files, inspect the replacement structure. When the residue only revives obsolete files or tests that assert stale paths, skip the preservation commit and remove the worktree/branch instead of resurrecting deleted implementation. See `references/pr-number-named-duplicate-and-obsolete-dirty-worktrees.md`.

...
If this is a repeated cleanup immediately after a preservation PR merged and `git diff origin/main` shows useful new guidance mixed with apparent deletions of files already on latest main, also read `references/repeated-cleanup-merged-preservation-pr-stale-deletions.md`. In that case, create a fresh latest-`origin/main` worktree and copy only existing authored payload; do not carry stale deletion hunks forward.

...
3. For Page indexing issue validation across all managed sites, prefer an all-site wrapper rather than a hard-coded site list.
   ...
   - In frontend-session mode, `Failed` rows can be valid candidates yet still fail before submission if the fetched index-page markup lacks `item_key`. Interpret `discovered=N candidates=M` plus `OUTCOME=FAIL` / `ACTION=item_key was not present...` as a direct-mode extraction failure, not as a decision to skip `Failed` validation. Re-export the frontend session from a fully loaded Page indexing table, and fix the direct helper's parser rather than silently falling back to browser control. See `references/gsc-frontend-item-key-missing.md`.

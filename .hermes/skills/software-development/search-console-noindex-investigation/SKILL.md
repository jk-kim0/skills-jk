...
3. For Page indexing issue validation across all managed sites, prefer an all-site wrapper rather than a hard-coded site list.
   ...
   - If repeated browser/CDP control makes the CLI impractical, split the implementation into a one-time `frontend-session export` step plus direct frontend HTTP calls that reuse saved cookies/WIZ tokens. The default `validate-index-issues` / `validate-index-issues-all` path must remain frontend-session-only: do not silently fall back to browser clicking on `--submit` failures. Keep browser/CDP control behind explicit `*-browser` commands only, so direct-mode failures surface as CLI errors that can be fixed in the frontend helper.
   - In frontend-session mode, `Failed` rows can be valid candidates yet still fail before submission if the fetched index-page markup lacks `item_key`. Interpret `discovered=N candidates=M` plus `OUTCOME=FAIL` / `started=0` / `ACTION=item_key was not present...` as a direct-mode extraction failure, not as a decision to skip `Failed` validation. If GSC still shows `Failed` after the run, explain that no validation request was sent because the CLI lacked a safe issue key/RPC target; a successful submit should transition the row to `Started` / `Validation started`. Re-export the frontend session from a fully loaded Page indexing table, and fix the direct helper's parser rather than silently falling back to browser control. See `references/gsc-frontend-item-key-missing.md`.
   - Current GSC Page indexing markup may omit row drilldown `href` / `item_key` even when `AF_initDataCallback` bootstrap data still contains issue keys keyed by support-article anchors. For direct-mode helpers, parse those callbacks as a fallback and map anchors/reasons to `itemKey` before opening validation pages. If the validation page exposes `START NEW VALIDATION` but no static RPC template, the current click-time RPC is `RYZlBc` with args `[site, 13, itemKey, 3, null, failedAt]`, where `failedAt` comes from validation-page bootstrap data. Browser/network interception is acceptable for debugging the RPC shape, but the final CLI implementation must reconstruct and send it from the saved authentication session only. Verify this by intercepting `/batchexecute` rather than sending a live request. See `references/gsc-frontend-failed-validation-rpc.md`.
   - If explicit browser/CDP mode is required for multiple properties, do not spawn one helper process per site. Use one helper process with repeated `--site` arguments so Chrome is attached once and the same WebSocket handles the full batch. Pair it with a persistent debug Chrome profile/port such as `~/.chrome-hermes-gsc` on `9333` to avoid repeated Chrome approval prompts. See `references/gsc-browser-batch-attach.md`.

...
- When reviewing all open issues in a repo, first fetch a single latest `origin/main` baseline and reuse it for the whole pass. Fast-forward the local `main` checkout when safe so the final report can state whether root `main` is current and clean. Then classify each issue into: still valid, partially valid / body stale, or effectively fixed / stale. This is especially useful for long-lived cleanup issues whose body names old file paths. Verify the exact cited paths still exist before trusting the issue scope; if the files are gone or moved but the underlying problem remains, rewrite the issue body around the current remaining scope instead of simply closing it.
- For small open-issue sweeps where the user explicitly asks to close resolved issues, treat each issue as needing a visible GitHub-side state update, not just a chat verdict. For resolved issues: rewrite the body into a latest-main completion record, add a closing evidence comment when useful, then close with `--reason completed`. For still-open issues: rewrite the body to start with the latest-main status, baseline SHA/time, open-PR state, and the concrete remaining mismatch so future work starts from current truth.
...
- Additional fast-moving-issue pitfall: if you rewrite an issue, then the user asks again shortly after, re-check latest `origin/main` and open PR state from scratch instead of assuming your just-written issue body is still current.
  - In active repos, a couple of follow-up PRs can merge between turns and flip the correct framing from "merge pending on open PRs" or "still open on latest main" to "already complete on latest main; close candidate".
  - This applies even when the same agent just edited the issue minutes earlier: fetch/pull latest `main`, inspect the newly merged PR/commit, and re-run the narrow file/content checks before answering.
  - When the newly merged commit resolves the only remaining mismatch, rewrite the issue body into a completion record with the new baseline SHA/time and merged PR evidence, add a closing evidence comment when useful, then close with `--reason completed` if the user asked to close resolved issues.
  - When that happens, update both body and title if needed so the issue stops reading like an active implementation tracker and instead becomes a completion-state record or close candidate.
...
- When rewriting a living feature-status/provider-smoke issue, also compare any linked status document's own baseline SHA/time against latest `origin/main`. If the status document is now stale, list that as a remaining documentation refresh item instead of silently treating the issue and status doc as synchronized. For external/provider work, distinguish code merged on `main`, environment/config evidence already documented on `main`, and live smoke evidence that still requires side effects.
- When related open PRs would change the issue's status, cite them explicitly as "not yet on latest main" and do not classify their scope as complete. This is especially important for doc/evidence PRs: an open PR can prove work is in progress, but the living issue requested "latest main" should remain grounded in `origin/main` only.

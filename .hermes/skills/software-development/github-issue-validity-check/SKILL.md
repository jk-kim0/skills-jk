...
- If the issue body or comments name specific follow-up PR numbers, check those PRs directly with `gh pr view <n> --json state,mergedAt,mergeCommit,title,url` even when they no longer appear in `gh pr list --state open`. A PR disappearing from the open list often means it merged, not that it became irrelevant.
- Practical stale-issue pattern: an issue body may say PRs like `#424` / `#425` are still open follow-ups, but after a fresh `main` update those PRs may already be merged and disappear from the open-PR list. Re-checking every named follow-up PR plus every currently open PR before editing prevents writing a second stale body.

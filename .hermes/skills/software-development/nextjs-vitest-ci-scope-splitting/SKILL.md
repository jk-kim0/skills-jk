...
- CI may report this through a job named `Validate Lint` even when ESLint itself passed: inspect the failed log for `AssertionError [ERR_ASSERTION]: Unassigned test files:` before editing application code. The aggregate `Validate Test` failure is often only a dependency summary of that earlier grouping failure.
- If the failure is `Unassigned test files: <path>`, update only the appropriate matcher in `scripts/ci/test-groups.mjs`, then run `node scripts/ci/assert-test-groups.mjs` or `npm run test:smoke` and the affected group script (for example `npm run test:routing`) before the full CI script.
- Component-section tests that render publication/resource-list UI, such as `src/components/sections/resource-list/__tests__/resource-list-load-more.component.test.tsx`, can belong in the `publications` group even though they are not under `src/lib/repo-content/**`; add a narrow `src/components/sections/resource-list/__tests__/` matcher rather than broadening all `src/components/sections/**`.

...
7. When the user names a specific mismatch (for example "H1 heading and lead description spacing"), measure that exact element pair with DOM geometry before changing anything. Also measure the nearest semantic/container wrapper (section/hero/content) and its computed padding/margins, because the heading and lead typography/gap can match while the parent section offset differs. Do not infer the fix from downstream landmarks such as card-grid start position.
   ...
   - When reviewing a PR that claims hero/header spacing parity, compare all three states before accepting or fixing it: the external reference, current production/main, and PR Preview. A PR can include the intended reference classes and still be a regression if the target header is in-flow and the title/lead are lower than the existing implementation. Prefer visible gap measurements (`header bottom → h1 top`, `h1 bottom → lead top`, `lead bottom → list/content top`) over class-string parity assertions.
...
9. For interactive controls inside those sections, measure every relevant state before claiming parity: normal, hover when applicable, programmatic `:focus`, keyboard `:focus-visible`, active/pressed if present, SVG child/path computed fill/stroke, and pseudo-elements. A CTA button is not visually matched until its icon color and focus ring/glow behavior match the reference, not only its normal-state size/background.
   ...
   - For header/GNB/drawer CTA text-style PRs, treat the visible text span as the contract, not just the clickable wrapper. Re-measure the live/reference page and PR Preview at the same desktop and mobile widths; compare exact `font-weight`, `color`, `line-height`, wrapper height, and mobile drawer height. If the reference span computes to values such as `font-weight: 400` and `#f6f6f6`, a PR changing the clone to `font-medium text-white` is a regression unless browser evidence says otherwise. See `references/cta-text-style-regression-pr-review.md`.
...
11. For shared chrome parity, audit header and footer as first-class UI surfaces, not just as present/absent landmarks. Run the chrome-specific JSON probe in `references/chrome-specific-parity-probe.md` on both pages at the same viewport, then compare the output before editing and again after the fix. Compare:
...
14. For shared chrome sub-surfaces such as a locale/language nudge banner, if computed styles already match across the exact pages and breakpoints, avoid speculative CSS edits. Inspect source duplication; when legacy and Tailwind route groups keep copied banner implementations, prefer a narrow shared-module refactor so both route groups use the same component/CSS and cannot drift again. See `references/shared-chrome-locale-banner-dedup.md`.
15. When a PR moves duplicated UI into a shared component across route groups, do not equate source sharing with UI safety. Check route-group global CSS imports, available CSS variables, Tailwind layers/resets, and nested atomic component computed styles. A shared wrapper can render the same geometry while a nested button loses its background because the Tailwind route group lacks legacy tokens. See `references/shared-component-route-group-compatibility.md`.
16. For MDX/publication detail parity, explicitly check whether the target renders raw MDX/JSX text. Literal tags such as `<Box>`, `<ArticleFileImage>`, `<br />`, or markdown links in the visible body mean the route is a body-preview stub, not a publication renderer. In that case, inspect the route for direct `{post.body}` / `renderBodyPreview` usage and fix it by evaluating MDX with the appropriate component map and composing the full article layout (hero, body images, TOC, related/sidebar CTAs), rather than patching typography around the raw text.
...
- [News detail body-to-footer gap measurement](references/news-detail-body-footer-gap-measurement.md) — landmark contract for measuring actual body bottom, padding wrapper, article section, and footer gaps without conflating internal section padding with external footer spacing.
...
- [Publication detail fixed vs in-flow header spacing](references/publication-detail-fixed-vs-inflow-header-spacing.md) — concrete article/news detail case: fixed reference header can require reducing target section top padding by the fixed header height when target header is sticky/in-flow.
...
- [News list hero parity](references/news-list-hero-parity.md) — measured `querypie.ai/news` source-component and browser geometry contract for aligning `/{locale}/news` title/lead size, position, and fixed-header spacing in corp-web-app.
- [Sidebar menu spacing parity](references/sidebar-menu-spacing-parity.md) — probe/fix pattern for matching left sidebar category/menu item spacing when one implementation uses container gap and another uses sibling margins or responsive utilities.
- [News/list hero component-contract parity](references/news-list-hero-component-contract-parity.md) — querypie.ai `/news` case: identify source component stack and port wrapper/title/lead/list-start classes as one coherent contract.
- [CTA text style regression PR review](references/cta-text-style-regression-pr-review.md) — focused probe/checklist for PRs that claim to align header/GNB/mobile CTA text but may make visible span typography worse.
...
- [Shared component route-group compatibility](references/shared-component-route-group-compatibility.md) — checklist for auditing whether a shared component actually renders the same across `(legacy)`/`(tailwind)` route groups with different global CSS/token environments.
- [Tailwind route cookie banner token parity](references/corp-web-app-tailwind-cookie-banner-token-parity.md) — case note for fixing shared legacy CookiePreference/Button/Text rendering inside a minimal Tailwind route group by scoping only the needed CSS variables to the banner rather than expanding Tailwind globals.

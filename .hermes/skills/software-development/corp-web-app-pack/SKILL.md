...
- `references/news-hero-image-metadata.md` — pattern for improving `/<locale>/news` and news detail metadata so Open Graph/Twitter images use explicit `openGraphImageSrc` before falling back to MDX/list hero images, with authoring-validation separation and focused Vitest verification.
...
- `references/component-name-debug-implementation.md` — implementation pattern for porting the Component Name Debug reviewer tool into `corp-web-app`: shared contract files, legacy/Tailwind header parity, overlay mounting, source-level tests, and pitfalls around marker-only wrappers and quote-sensitive assertions.
- `references/component-debug-production-preview-toggle-boundary.md` — boundary pattern for keeping Component Name Debug production availability separate from Preview Toggle production UI exposure; use a separate debug control and OpenSpec contract when production visitors must not see Preview mode controls.
...
- `references/news-hero-image-text-localization.md` — workflow for editing an existing Korean/source news hero PNG into localized EN/JA variants while preserving composition, dimensions, gradients, and MDX-referenced filenames.
- `references/news-docx-press-release-ingestion.md` — workflow for turning a source DOCX press release into a news MDX record: extract document text/images, map DOCX image order to `hero-<locale>.png` and `ArticleFileImage`, use `docs/news.md` canonical company/media-contact blocks, update news counts, and verify with source-level checks.
...
- `references/news-list-mobile-card-order.md` — source-level pattern for fixing `/<locale>/news` mobile card order when Tailwind `flex-col-reverse` makes thumbnails render above date/title/description, while preserving desktop `md:flex-row`.
...
- `references/pr-dirty-rebase-already-applied-commit.md` — pattern for fixing corp-web-app PRs whose GitHub merge state is `DIRTY` because earlier stacked commits already landed on `main`: rebase onto latest `origin/main`, accept skipped already-applied commits when appropriate, verify the remaining scoped diff, force-push with lease, and treat immediate `BLOCKED` as pending checks rather than unresolved conflicts.
- `references/tailwind-named-component-rebase-conflicts.md` — pattern for rebasing Tailwind named-component extraction PRs when latest `main` added Component Name Debug markers/tests: keep `page.tsx` thin, preserve latest-main markers inside extracted components, merge test source constants, verify conflict markers/diff check, and avoid long passive `gh pr checks` waits after force-push.
...
17. For Component Name Debug follow-ups, do not conflate production inclusion of the debug feature with production exposure of Preview Toggle UI. Preview Toggle remains non-production-only; if production needs a debug mode selector, use an independent Component Name Debug control and record the boundary in OpenSpec. See `references/component-debug-production-preview-toggle-boundary.md`.

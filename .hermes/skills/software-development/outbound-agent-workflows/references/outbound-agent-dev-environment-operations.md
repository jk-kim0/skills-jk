...
- `references/main-churn-interim-deploy-diagnosis.md` — Interim status pattern when Vercel is Ready/Building for successive SHAs while Tencent Seoul/Tokyo jobs are cancelled, skipped, pending, or not yet started.
...
- `references/three-environment-deploy-verification.md` — Concrete post-merge verification pattern across Vercel, dev-seoul, and dev-tokyo, including Vercel commit-metadata lookup, `gh api` polling, and manual Tokyo smoke when needed.

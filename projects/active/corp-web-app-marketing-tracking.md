...
Plans 페이지(`/plans`, `/en/plans`, `/ko/plans`, `/ja/plans`)의 구독·구매 의도 버튼 클릭 시 `select_plan` 이벤트 전송.

| 버튼 | plan_name 값 | 비고 |
|------|-------------|------|
| Subscribe ($20/mo) | `aip_starter` | app.querypie.com으로 이동 |
| Subscribe ($500/mo) | `aip_team` | app.querypie.com으로 이동 |
...
**완료 기준:** GA4 > Acquisition > Traffic acquisition 리포트에서 utm_campaign별 Conversions 컬럼 확인 가능 (`generate_lead` + `select_plan` 모두)

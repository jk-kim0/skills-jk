...
> | 2 | Plans 페이지 버튼 클릭 | Subscribe / Contact Us 등 구독·구매 의도 버튼 클릭 | `select_item` |
...
**Conversion 2 — Plans 페이지 버튼 클릭:**

Plans 페이지(`/plans`, `/en/plans`, `/ko/plans`, `/ja/plans`)의 구독·구매 의도 버튼 클릭 시 `select_item` 이벤트 전송.

| 버튼 | plan_name 값 | 비고 |
|------|-------------|------|
| Subscribe ($20/mo) | `aip_starter` | app.querypie.com으로 이동 |
| Subscribe ($500/mo) | `aip_pro` | app.querypie.com으로 이동 |
| Try Now | `aip_enterprise` | app.querypie.com으로 이동 |
| Install Now | `acp_community` | docs.querypie.com으로 이동 |
| Contact Us (Standard) | `acp_standard` | /company/contact-us로 이동 |
| Contact Us (Enterprise) | `acp_enterprise` | /company/contact-us로 이동 |

- `Plan.Button`에 `gaEvent` prop 연결 필요 (`ButtonLink`는 이미 지원)
- GA4 Admin에서 `select_item`을 Conversion으로 마킹

**완료 기준:** GA4 > Acquisition > Traffic acquisition 리포트에서 utm_campaign별 Conversions 컬럼 확인 가능 (`generate_lead` + `select_item` 모두)

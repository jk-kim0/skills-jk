...
www.querypie.com 방문자의 마케팅 유입 경로(UTM)를 추적하고, **캠페인별 전환율(리드 수집 · ACP Community 설치)을 측정**하여 어떤 채널·캠페인·광고 소재가 실제 전환을 만드는지 파악한다.
...
> | 2 | Plans 페이지 버튼 클릭 | Install Now (Community) 포함 모든 플랜 선택 버튼 클릭 | `select_plan` |
>
> **ACP 제품 특이사항:** Community Edition(무료)은 고비용 B2B 제품 구매 전 단계로, ACP 디지털 마케팅의 **1차 전환 목표**다. Enterprise/Standard 구매 문의보다 빈도가 훨씬 높으며, Community 이용률 확대가 ACP 성장의 핵심 지표다.
>
> Community Edition 채택은 두 가지 지표를 **각각 독립적으로** 집계한다:
> - **Install Now 클릭** (`select_plan`, `plan_name=acp_community`): 설치 의향 표명 (Plans 페이지 버튼)
> - **라이선스 신청 폼 제출** (`generate_lead`, `form_type=community_license`): 실제 신청 완료
...
- "어떤 landing page의 전환율이 낮은가? → 광고 랜딩 페이지 교체·최적화로 같은 예산에서 더 많은 리드 확보" (`generate_lead`)
- "어떤 채널 유입이 ACP Community Edition 설치(Install Now)로 가장 많이 이어지는가?" (`select_plan`, `plan_name=acp_community`)
- "어떤 채널 유입이 ACP Community 라이선스 신청(폼 제출)으로 가장 많이 이어지는가?" (`generate_lead`, `form_type=community_license`)
- "Plans 페이지에서 어떤 채널 유입이 플랜 선택으로 가장 많이 이어지는가?" (`select_plan`)
- "같은 캠페인에서 어떤 광고 소재·메시지(`utm_content`)의 전환율이 높은가?" (`generate_lead` + `select_plan`)
...
> **Phase 2A · 2B는 독립적으로 병렬 진행 가능하다.** 서로 다른 컴포넌트(폼 vs Plans 페이지 버튼)에 구현하며 의존 관계가 없다.

### 데이터 흐름 (현재 → 목표)

```
방문자 UTM URL 접속
  ├─ GA4 세션 자동 기록 (utm_source/medium/campaign/content 포함) ← 이미 동작
  └─ utm-attribution 쿠키 저장 (utm_content 포함) ← Phase 1 완료
...
3. GA4 Admin에서 `form_type`을 **Custom Dimension**으로 등록
   - 미등록 시 표준 Acquisition 리포트에서 form_type 필터·보조 측정기준 사용 불가
...
**목표:** Plans 페이지의 모든 플랜 선택 버튼 클릭을 GA4 전환 이벤트로 등록해, 채널별 플랜 선택률을 확인할 수 있게 한다. ACP Community Edition(Install Now)은 ACP 디지털 마케팅의 1차 전환 목표로, `plan_name` 파라미터로 플랜별 분리 측정이 핵심이다.
...
3. GA4 Admin에서 `plan_name`을 **Custom Dimension**으로 등록
   - 미등록 시 표준 Acquisition 리포트에서 plan_name 필터·보조 측정기준 사용 불가 (Community Edition 채널별 분리 측정 불가)
...
**상태: 미착수** (Phase 2A 또는 2B 중 하나 완료 시 해당 리포트부터 즉시 운영 가능)

### Phase 3A — GA4 기본 리포트 운영

Phase 2A 또는 2B 완료 시 해당 이벤트 리포트부터 추가 구현 없이 즉시 확인 가능:
...
**완료 기준:** 캠페인 담당자가 GA4 기본 리포트에서 최종 목표 상태의 8개 질문에 모두 답할 수 있음
...
| 차트 | 측정 항목 |
|------|----------|
| 채널별 세션 → 전환 퍼널 | 세션 수 / generate_lead 전환 수 / 전환율 |
| ACP Community 채널별 설치 유도율 | 채널별 select_plan(acp_community) 전환 수·전환율 |
| 캠페인별 전환 수 추이 | utm_campaign별 전환 건수 (월별) |
| 메시지별 전환율 비교 | utm_content별 전환율 (A/B 메시지 성과 비교) |

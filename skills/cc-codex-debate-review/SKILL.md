...
2. 프롬프트 템플릿 읽기: `skills/cc-codex-debate-review/codex-lead-review-prompt.md`
3. 플레이스홀더 치환: `{REPO}`, `{PR_NUMBER}`, `{PR_TITLE}`, `{PR_BODY}`, `{ROUND}`, `{REVIEW_CONTEXT}`, `{OPEN_ISSUES}`, `{PENDING_REBUTTALS}`, `{REVIEW_CRITERIA}`, `{DIFF}`
4. PR diff 획득:
   ```bash
...
프롬프트 템플릿: `skills/cc-codex-debate-review/codex-cross-verify-prompt.md`
...
프롬프트 템플릿: `skills/cc-codex-debate-review/codex-lead-response-prompt.md`
...
- `{REVIEW_CRITERIA}`: `skills/cc-codex-debate-review/review-criteria.md` 파일 내용

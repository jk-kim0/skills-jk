...
설정 파일에서 `max_rounds`, `codex_sandbox` 등을 읽는다.
...
3. Step 1: Lead agent 리뷰
4. Step 1a: 미결 rebuttal 처리
5. Step 2: Cross-verifier 교차 검증 (clean pass 시 생략)
6. Step 3: Lead agent 응답 + 코드 반영 (clean pass 시 생략)
7. Step 4: 정산
...
REPO_ROOT=$(jq -r '.repo_root' < <(bin/debate-review show --state-file "$STATE_FILE" --json))
WORKTREE_PATH="$REPO_ROOT/.worktrees/debate-pr-$PR_NUMBER"
HEAD_BRANCH=$(jq -r '.head_branch' < <(bin/debate-review show --state-file "$STATE_FILE" --json))
...
5. Codex 실행:
   ```bash
   cd "$WORKTREE_PATH"
   codex exec -s danger-full-access "$FILLED_PROMPT"
   ```
...
2. Cross-verifier의 새 findings 평가 (accept/rebut)
...
Codex가 lead이고 `code_fixes` 배열이 비어 있지 않으면, CC가 각 diff를 워크트리에 적용:

```bash
for fix in code_fixes:
  echo "$fix_diff" | git -C "$WORKTREE_PATH" apply --check  # 검증
  echo "$fix_diff" | git -C "$WORKTREE_PATH" apply           # 적용
```

적용 실패 시 해당 issue는 `failed-issues`로 기록. CC가 lead일 때는 CC가 직접 코드를 수정한다.
...
**범위:** `max(1, current_round - 2)` ~ `current_round - 1`의 completed/superseded 라운드. Round 1에서는 생략.
...
호출 패턴:
1. 프롬프트 템플릿 읽기
2. 플레이스홀더 치환 (`{REPO}`, `{PR_NUMBER}`, `{ROUND}` 등)

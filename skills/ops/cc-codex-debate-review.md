...
`--step "3"`에서는 `accept`가 lead의 cross findings 수락을 의미한다.
...
git -C "$REPO_ROOT" worktree remove "$REPO_ROOT/.worktrees/debate-pr-$PR_NUMBER" --force
```

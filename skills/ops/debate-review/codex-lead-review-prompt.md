...
- findings가 **0건**이고 이전에 제기된 모든 open issue가 해소되었으며, accepted issue가 있다면 모두 반영 완료 상태여야 함 → verdict를 `no_findings_mergeable`로 설정
- fork PR에서는 push 대신 recommendation으로 종료되므로, accepted issue의 `application_status=recommended`도 clean pass로 간주함
